...
def split_qupath_roi_folder(roi_folder):
	...
	for gj_f in geojson_files:
		with open(join(roi_folder, gj_f)) as file:
			...
			# HE dimensions
			dim_plt = [int(geo_json["dim"]["width"]), int(geo_json["dim"]["height"])]
			tma_name = geo_json["name"]

			if "featureNames" in geo_json:
...
if __name__ == "__main__":
	...
	parser.add_argument("--fill", action="store_true", required=False, help="Fill holes in ROI (order of coordinates will be lost)")
